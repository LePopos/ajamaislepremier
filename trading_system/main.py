...
def initialize_clients(logger) -> tuple[TipRanksClient, YFinanceClient, Optional[NotificationService]]:
    ...
    return tipranks_client, yfinance_client, notification_service
        ...
        if signals:
            ...
            for signal_type, signal_list in signal_summary.items():
                ...
                for signal in signal_list:
                    # Get stock ticker from the same session
...
def main():
    ...
    try:
        tipranks_client, yfinance_client, notification_service = initialize_clients(logger)
    except Exception as e:
        logger.error(f"Failed to initialize API clients: {e}")
        sys.exit(1)
    
    # Initialize services
    data_collector = DataCollector(tipranks_client, yfinance_client)
    signal_generator = SignalGenerator(tipranks_client, yfinance_client, config.__dict__, notification_service)
    ...
    try:
        if args.command == 'collect':
            ...
            print(f"  YFinance: ✓ (no API key needed)")
